...
外部 PR 状态最近一次核验：`2026-09-24`。其中 `#3419` 只提供有边界的互操作示例：交接保持只读，不创建执行授权、治理决定、生产部署或 Microsoft 对 SAEE/TITMAS 的整体采用结论。

## 上游后续采纳的贡献

### INGV：跨数据中心事件身份连续性

[PR #1](https://github.com/INGV/mcp-fdsnws-event/pull/1) 提出跨数据中心
`(datacenter, EventID)` 身份连续性 A/B 验证思路和结果分类器设计。
原 PR 面向 1.x API，已于 2026 年 9 月 23 日关闭，**未合并**。
维护者在 [Issue #2](https://github.com/INGV/mcp-fdsnws-event/issues/2) 延续该工作，
并在 [2.x 适配提交 `6fa74bd`](https://github.com/INGV/mcp-fdsnws-event/commit/6fa74bddc6249b8b8280f8d4349d113730308bed)
中明确署名引用我们的思路和分类器设计；该提交已进入 `develop`。

维护者报告的两个模型、160 次尝试未显示新增身份绑定句带来可测改善，
因此没有修改生产工具描述。这证明有边界的研究思路与测试设计被后续采纳，
不代表原 PR 已合并、模型可靠性获得改善或整套框架被采用。
